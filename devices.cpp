...
const std::vector<const char *> PhysicalDeviceManager::deviceExtensions = {VK_KHR_SWAPCHAIN_EXTENSION_NAME};

vector<tuple<VkPhysicalDevice, VkPhysicalDeviceProperties, VkPhysicalDeviceFeatures>>
PhysicalDeviceManager::getAvailableDevices(VkInstance *instance) {
...
bool PhysicalDeviceManager::checkDeviceExtensionSupport(VkPhysicalDevice device) {
  ...
  set<std::string> requiredExtensions(PhysicalDeviceManager::deviceExtensions.begin(),
...
QueueFamilyIndices PhysicalDeviceManager::findSuitableQueueFamiles(VkPhysicalDevice device,
...
vector<VkDeviceQueueCreateInfo> PhysicalDeviceManager::buildQueueCreateInfos(QueueFamilyIndices indices) {
...
SwapChainSupportDetails PhysicalDeviceManager::querySwapChainSupport(VkPhysicalDevice *device,
                                                                     VkSurfaceKHR *surface) {
...
PhysicalDeviceManager::pickPhysicalDevice(VkInstance *instance, VkSurfaceKHR *surface,
  ...
  int idx = 0;
  auto availableDevices = PhysicalDeviceManager::getAvailableDevices(instance);
  for (tuple<VkPhysicalDevice, VkPhysicalDeviceProperties, VkPhysicalDeviceFeatures> &dev :
    ...
    QueueFamilyIndices indices = PhysicalDeviceManager::findSuitableQueueFamiles(get<0>(dev), surface);

    // check swap chain support
    SwapChainSupportDetails swapChainSupport = querySwapChainSupport(&get<0>(dev), surface);
  ...
  QueueFamilyIndices indices = PhysicalDeviceManager::findSuitableQueueFamiles(selectedDevice, surface);

  auto queueInfos = PhysicalDeviceManager::buildQueueCreateInfos(indices);
